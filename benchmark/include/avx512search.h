...
// Experiment: after the four peeled compares, if exactly ONE lane survives, stop
// narrowing and verify that candidate directly. The profile shows the narrowing
// loop (j >= 4) is ~30% of samples on a 1 kB haystack, and it runs essentially
// only at the matching window -- where it pays a full 64-byte load and masked
// compare per needle byte to confirm a single live lane.
//
// The one-survivor guard is what keeps this safe on adversarial input: when the
// filter fails, every lane survives, blsr is non-zero, and we fall back to the
// original narrowing loop. So this cannot turn into the anchored kernel's
// per-candidate blow-up.
std::pair<bool, size_t> avx512_naive_search_blsr(const char* text, size_t n, const char* pattern, size_t m) {
    if (m == 0) return {true, 0};
    if (n < m) return {false, 0};
    const size_t step = 64;
    size_t i = 0;
    if (n >= m + 63 && m >= 4) {
        for (; i + m + 63 <= n; i += step) {
            ...
            found = _mm512_mask_cmpeq_epi8_mask(
                found, _mm512_loadu_si512((const void*)(text + i + 3)), p3);
            if (found == 0) continue;
            if ((found & (found - 1)) == 0) {          // BLSR: exactly one survivor
                size_t b = (size_t)__builtin_ctzll(found);
                if (m == 4 || std::memcmp(text + i + b + 4, pattern + 4, m - 4) == 0)
                    return {true, i + b};
                continue;
            }
            for (size_t k = 4; k < m; ++k) {           // many survivors: narrow
                if (found == 0) break;
                __m512i pk = _mm512_set1_epi8((char)pattern[k]);
                found = _mm512_mask_cmpeq_epi8_mask(
                    found, _mm512_loadu_si512((const void*)(text + i + k)), pk);
            }
            if (found == 0) continue;
            return {true, i + (size_t)__builtin_ctzll(found)};
        }
    }
    // Masked epilogue instead of a scalar tail. The main loop stops while a full
    // 64-byte load is still in bounds, leaving at most 63 candidate positions.
    // One predicated window covers them: `active` marks the lanes that are real
    // candidates (i + b + m <= n), and maskz loads touch only those lanes, so
    // nothing out of bounds is ever read. This is the same discipline the
    // anchored kernel already uses.
    // Masked epilogue instead of a scalar tail. This is a LOOP, not a single
    // window: the main loop may not have run at all (m < 4, or a haystack
    // shorter than one full window), in which case every candidate position is
    // left here. `active` marks the lanes that are real candidates
    // (i + b + m <= n) and maskz loads touch only those, so nothing out of
    // bounds is read. Same discipline the anchored kernel already uses.
    for (; i + m <= n; i += 64) {
        const size_t cand = n - m - i + 1;
        __mmask64 active = (cand >= 64) ? ~(__mmask64)0
                                        : (((__mmask64)1 << cand) - 1);
        __mmask64 found = active;
        for (size_t k = 0; k < m && found != 0; ++k) {
            __m512i pk = _mm512_set1_epi8((char)pattern[k]);
            found = _mm512_mask_cmpeq_epi8_mask(
                found, _mm512_maskz_loadu_epi8(found, text + i + k), pk);
        }
        if (found != 0) return {true, i + (size_t)__builtin_ctzll(found)};
    }
    return {false, 0};
}

std::pair<bool, size_t> avx512_naive_search_ovl(const char* text, size_t n, const char* pattern, size_t m) {
    if (m == 0) return {true, 0};
    if (n < m) return {false, 0};
    const size_t step = 64;
    size_t i = 0;
    ...
    // Final pass without masks: back the window up to the last legal offset and
    // do one FULL 64-byte window. It re-scans positions the main loop already
    // cleared, which is harmless -- they held no match, or we would have returned
    // -- and it costs no mask arithmetic. Only a haystack too small for even one
    // full window still needs the predicated loop.
    if (i + m <= n) {
        if (m >= 4 && n >= m + 63) {
            const size_t last = n - m - 63;
            __mmask64 found = _mm512_cmpeq_epi8_mask(
                _mm512_loadu_si512((const void*)(text + last)),
                _mm512_set1_epi8((char)pattern[0]));
            for (size_t k = 1; k < m && found != 0; ++k) {
                __m512i pk = _mm512_set1_epi8((char)pattern[k]);
                found = _mm512_mask_cmpeq_epi8_mask(
                    found, _mm512_loadu_si512((const void*)(text + last + k)), pk);
            }
            if (found != 0) return {true, last + (size_t)__builtin_ctzll(found)};
        } else {
            for (; i + m <= n; i += 64) {
                const size_t cand = n - m - i + 1;
                __mmask64 active = (cand >= 64) ? ~(__mmask64)0
...
// Two changes over _ovl, both taken from what glibc's strstr does:
//
//  1. The four peeled compares are INDEPENDENT and combined with an AND tree,
//     instead of a chain of masked compares each reading and writing the same
//     mask register. The chain serialises: four dependent VPCMPB back to back,
//     ~50% of profiled cycles. Independent compares issue in parallel and only
//     the AND tree is dependent.
//  2. Verification is inline. For m <= 64 the needle is preloaded once into a
//     register and a candidate costs one masked compare, instead of calling
//     std::memcmp, which showed up as 12% of cycles in __memcmp_evex_movbe.
static inline __attribute__((always_inline)) std::pair<bool, size_t>
avx512_naive_search_v2_body(const char* text, size_t n,
    ...
    if (m >= 4) {
        ...
        for (; i + m + 63 <= n; i += 64) {
            const __mmask64 c0 = _mm512_cmpeq_epi8_mask(
                _mm512_loadu_si512((const void*)(text + i + 0)), p0);
            const __mmask64 c1 = _mm512_cmpeq_epi8_mask(
                _mm512_loadu_si512((const void*)(text + i + 1)), p1);
            const __mmask64 c2 = _mm512_cmpeq_epi8_mask(
                _mm512_loadu_si512((const void*)(text + i + 2)), p2);
            const __mmask64 c3 = _mm512_cmpeq_epi8_mask(
                _mm512_loadu_si512((const void*)(text + i + 3)), p3);
            ...
            while (f != 0) {
                ...
                if (fits) {
                    if (_mm512_mask_cmpneq_epi8_mask(
                            nmask, _mm512_maskz_loadu_epi8(nmask, text + i + b),
                            nvec) == 0)
                        return {true, i + b};
                } else if (std::memcmp(text + i + b, pattern, m) == 0) {
                    return {true, i + b};
                }
                f &= f - 1;
            }
...
// Out-of-line instance for the algorithm table.
std::pair<bool, size_t> avx512_naive_search_v2(const char* text, size_t n,
                                               const char* pattern, size_t m) {
    return avx512_naive_search_v2_body(text, n, pattern, m);
}
...
// Resolve one 64-byte chunk whose peeled filter left survivors. A single
// survivor -- the usual case at the matching block -- is verified directly
// instead of narrowing, which would cost a full 64-byte load and masked compare
// per remaining needle byte to confirm one live lane. Several survivors means
// the filter failed, which is the adversarial case, and falls back to narrowing.
static inline bool avx512_resolve_chunk(const char* q, const char* pattern,
                                        size_t m, __mmask64 f, size_t& out) {
    if ((f & (f - 1)) == 0) {
        const size_t b = (size_t)__builtin_ctzll(f);
        if (m == 4 || std::memcmp(q + b + 4, pattern + 4, m - 4) == 0) { out = b; return true; }
        return false;
    }
    for (size_t k = 4; k < m && f != 0; ++k)
        f = _mm512_mask_cmpeq_epi8_mask(
                f, _mm512_loadu_si512((const void*)(q + k)),
                _mm512_set1_epi8((char)pattern[k]));
    if (f != 0) { out = (size_t)__builtin_ctzll(f); return true; }
    return false;
}

// Wide-stride kernel with the single-survivor fast path and no scalar tail: the
// 256-byte loop hands off to a 64-byte loop and then to a predicated window, so
// every byte is covered by SIMD.
std::pair<bool, size_t> avx512_naive_search256_opt(const char* text, size_t n,
                                                   const char* pattern, size_t m) {
    if (m == 0) return {true, 0};
    if (n < m) return {false, 0};
    size_t i = 0, b;
    ...
    if (m >= 4) {
        for (; i + m + 255 <= n; i += 256) {
            ...
            if ((fA | fB | fC | fD) == 0) continue;
            if (fA && avx512_resolve_chunk(text+i+  0, pattern, m, fA, b)) return {true, i+  0+b};
            if (fB && avx512_resolve_chunk(text+i+ 64, pattern, m, fB, b)) return {true, i+ 64+b};
            if (fC && avx512_resolve_chunk(text+i+128, pattern, m, fC, b)) return {true, i+128+b};
            if (fD && avx512_resolve_chunk(text+i+192, pattern, m, fD, b)) return {true, i+192+b};
        }
        for (; i + m + 63 <= n; i += 64) {          // 64-byte stride for the remainder
            const __m512i p0 = _mm512_set1_epi8((char)pattern[0]);
            __mmask64 f = _mm512_cmpeq_epi8_mask(_mm512_loadu_si512((const void*)(text+i)), p0);
            for (size_t k = 1; k < 4 && f != 0; ++k)
                f = _mm512_mask_cmpeq_epi8_mask(
                        f, _mm512_loadu_si512((const void*)(text+i+k)),
                        _mm512_set1_epi8((char)pattern[k]));
            if (f == 0) continue;
            if (avx512_resolve_chunk(text+i, pattern, m, f, b)) return {true, i+b};
        }
    }
    for (; i + m <= n; i += 64) {                   // predicated window, no scalar tail
        const size_t cand = n - m - i + 1;
        __mmask64 active = (cand >= 64) ? ~(__mmask64)0 : (((__mmask64)1 << cand) - 1);
        __mmask64 f = active;
        for (size_t k = 0; k < m && f != 0; ++k)
